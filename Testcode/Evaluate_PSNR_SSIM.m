...
for idx_method = 1:num_method
        record_results_txt = [methods{idx_method},'_Results_', num2str(scale_all), '_model.txt'];
    ...
    for idx_set = 1:num_set
        ...
        for scale = scale_all
            ...
            for idx_im = 1:length(filepaths)
                ...
                name_SR = strrep(name_HR, 'HR', 'SRDDN');
                im_HR = imread(fullfile('./HR', dataset{idx_set}, ['x', num2str(scale)], name_HR));
                im_SR = imread(fullfile('./SR', methods{idx_method},dataset{idx_set}, ['x', num2str(scale)], name_SR));
